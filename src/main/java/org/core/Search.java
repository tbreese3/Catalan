...
public final class Search {
	...
	private int negamax(long[] board, int depth, int ply, int alpha, int beta, boolean pvNode) {
		...
		if (PositionFactory.isDraw(board)) return 0;

		int bucket = TranspositionTable.TT.probeBucket(PositionFactory.zobrist(board));
		int slot = bucket >= 0 ? TranspositionTable.TT.findSlotInBucket(bucket, PositionFactory.zobrist(board)) : -1;
		...
		if (ttHit) {
			...
			int halfMoves = PositionFactory.halfmoveClock(board);
		...
		boolean inCheck = PositionFactory.isInCheck(board);
		se.inCheck = inCheck;
		...
		int moveCount = MoveGenerator.generateCaptures(board, moves, 0);
		moveCount = MoveGenerator.generateQuiets(board, moves, moveCount);
		...
		if (se.staticEval == SCORE_NONE) {
			...
			TranspositionTable.TT.store(PositionFactory.zobrist(board), (short) 0, TranspositionTable.SCORE_NONE_TT, rawEval, TranspositionTable.BOUND_NONE, 0, pvNode, false);
		}
		...
		for (int i = 0; i < moveCount; i++) {
			...
			if (!PositionFactory.makeMoveInPlace(board, move)) { Eval.undoMoveAccumulator(nnueState); continue; }
			movePlayed = true;
		...
		TranspositionTable.TT.store(PositionFactory.zobrist(board), (short) (bestMove & 0xFFFF), storeScore, rawEval, bound, depth, pvNode, pvNode || prevWasPV);
	...
	private int quiescence(long[] board, int ply, int alpha, int beta, boolean pvNode) {
		...
		int moveCount;
		if (inCheck) {
			moveCount = MoveGenerator.generateCaptures(board, moves, 0);
			moveCount = MoveGenerator.generateQuiets(board, moves, moveCount);
		} else {
			moveCount = MoveGenerator.generateCaptures(board, moves, 0);
		}
		...
		int bestMove = se.pvLength > 0 ? se.pv[0] : 0;
		boolean prevWasPV = ttHit && TranspositionTable.TT.readWasPV(bucket, slot);
		TranspositionTable.TT.store(PositionFactory.zobrist(board), (short) (bestMove & 0xFFFF), storeScore, rawEval, bound, 0, pvNode, pvNode || prevWasPV);
