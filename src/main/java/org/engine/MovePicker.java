	...
	int next() {
	    ...
	    case KILLER: {
					...
					if (!counterTried && !MoveFactory.isNone(counterMove) && counterMove != ttMove && counterMove != killerMove) {
						counterTried = true;
						// Only try if it's a quiet move (not capture, promo, or castle)
						int flags = MoveFactory.GetFlags(counterMove);
						...
						int to = MoveFactory.GetTo(counterMove);
						int targetPiece = isEP ? (PositionFactory.whiteToMove(board) ? 6 : 0) : PositionFactory.pieceAt(board, to);
						boolean isCapture = isEP || (targetPiece != -1);
						if (!isPromotion && !isCastle && !isCapture) {
