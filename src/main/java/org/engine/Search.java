...
public final class Search {
	...
	private int negamax(long[] board, int depth, int ply, int alpha, int beta, NodeType nodeType) {
		...
		se.ttHit = ttHit;
		int prevWasPVBit = ttHit ? (TranspositionTable.TT.readWasPV(bucket, slot) ? 1 : 0) : 0;
		se.ttPV = isPVNode || (prevWasPVBit == 1);
		...
		if (!inCheck) {
			...
			boolean storeWasPV = isPVNode || prevWasPV;
			TranspositionTable.TT.store(pos.zobrist(board), (short) 0, TranspositionTable.SCORE_NONE_TT, rawEval, TranspositionTable.BOUND_NONE, 0, isPVNode, storeWasPV);
		}
		...
		// Propagate PV flag on fail-low like reference code
